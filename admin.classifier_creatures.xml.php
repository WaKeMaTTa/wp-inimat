...
if($form->validate()) {
	...
	if (!$query <= 0) {
		...
		if ( isset($form->file_upload["imgSketch"]) ) {
			...
			$sql_imgSketch = $form->file_upload["imgSketch"]["file_name"];
		...
		if ( isset($form->file_upload["imgModeled"]) ) {
			...
			$sql_imgModeled = $form->file_upload["imgModeled"]["file_name"];
		...
		if ( isset($form->file_upload["imgTextured"]) ) {
			...
			$sql_imgTextured = $form->file_upload["imgTextured"]["file_name"];
		...
		if ( isset($form->file_upload["file"]) ) {
			...
			$sql_file = $form->file_upload["file"]["file_name"];
		...
		for($i=1; $i<=$_POST["skill_total"]; $i++) {
			...
			$arrya_skills[$i][0] = $_POST["skill_".$i];
			$arrya_skills[$i][1] = $_POST["type_skill_".$i];
			
			if ($_POST["skill_".$i] != '') {
				
				$skill = str_replace(',', ' ', $_POST["skill_".$i]);
				
				$type_skill = str_replace(',', ' ', $_POST["type_skill_".$i]);
				
				$sql_skills .= strtolower($skill) . ',' . $type_skill . ',';
				
			}
			
		}
		
		$arrya_skills_serial = serialize($arrya_skills);
		...
		$query = $wpdb -> query( 
			...
			$wpdb -> prepare("
				INSERT INTO ".$wpdb->prefix."inimat_creatures ( date_add, id_author, name_author, name, height, width, weight, type, gender, skills, habitat, description, sketch, modeled, textured, file, license, finished ) VALUES ( CURRENT_TIMESTAMP, %d, %s, %s, %f, %f, %f, %s, %s, %s, %s, %s, %s, %s, %s, %s, %d, %d )",
				$sql_id_author, $sql_name_author, $_POST["name"], $_POST["height"], $_POST["width"], $_POST["weight"], $_POST["type"], $_POST["gender"], $sql_skills, $_POST["habitat"], $_POST["description"], $sql_imgSketch, $sql_imgSketch, $sql_imgModeled, $sql_file, $_POST["license"], $sql_finished
		...
		if (WP_DEBUG == TRUE) {
			...
			echo '<p style="font-size: 24px;">Debug Mode:</p>';
			
			show_results();
			
			print_r($arrya_skills_serial);
